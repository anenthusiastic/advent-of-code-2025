# Advent of Code 2025 – Topluluk Çözümleri

Bu repo, topluluğumuzun Advent of Code - 2025 çözümlerini paylaşması için hazırlanmıştır.
Her gün için tek bir klasör bulunur ve herkes çözümlerini kendi ismiyle adlandırılmış bir dosya olarak ekler.

## Nasıl Katkı Yapılır?

Repoyu forklayın.

Kendi fork’unuzda ilgili günün klasörüne dosyanızı ekleyin.

Değişikliklerinizi commit’leyip fork’unuzdan bu repo'nun `main` branch’ine bir Pull Request açın.

Pull Request açarken:
- PR başlığı şu formatta olabilir: `gün X: kullaniciAdi çözümü`
- Açıklama kısmına isterseniz çözüme dair not ekleyebilirsiniz.

### Dosya isimlendirme

`githubKullaniciAdi.example`

Örnek:
- day1/ahmet.go
- day2/ayse.js

Part ayırmak isterseniz:
- kullaniciAdi_part1.go
- kullaniciAdi_part2.go

## Kurallar ve Öneriler

- Lütfen günün çözümlerini gün bitmeden önce göndermeyin. İlgili gün tamamlandıktan sonra çözümlerinizi istediğiniz zaman yollayabilirsiniz.

- Tartışmalar için [Discussions](https://github.com/Bilgisayar-Kavramlari-Toplulugu/advent-of-code-2025/discussions) kullanabilirsiniz. Her güne ait bir tartışma başlığı açılacaktır. Spoiler'dan kaçınmak için günün çözümünü bitirmeden okumamanızı öneririz. :)

- Her günün çözümleri tek bir klasörde, katkı yapan kişinin adıyla adlandırılmış dosyalar halinde bulunur.

- Herkes istediği dili kullanabilir.

- **Lütfen input dosyalarınızı paylaşmayın.** Advent of Code'un kullanım şartlarında paylaşım yasaktır.

- Part 1 ve Part 2 için tek dosya veya iki ayrı dosya kullanabilirsiniz.

- Aynı gün için birden fazla yaklaşımınız varsa `kullaniciAdi_v2.ext` gibi versiyonlayabilirsiniz.
